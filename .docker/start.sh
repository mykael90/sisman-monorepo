...
# /home/node/sisman-monorepo/.docker/create-directories.sh

# Condicionais baseadas no valor de ENV
if [ "$ENV" == "dev" ]; then
  echo "Ambiente de desenvolvimento detectado. Rodando script de desenvolvimento..."
  /home/node/sisman-monorepo/.docker/start-dev.sh  # Substitua pelo script para o ambiente de dev
elif [ "$ENV" == "prod" ]; then
  echo "Ambiente de produção detectado. Rodando script de produção..."
  /home/node/sisman-monorepo/.docker/start-prod.sh  # Substitua pelo script para o ambiente de prod
else
