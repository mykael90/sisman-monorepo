...
export type IMaterialReceiptItemAddFormInfoWithdrawal = Pick<
  IMaterialReceiptItemAddForm,
  'key' | 'materialId'
> &
  Pick<IMaterialGlobalCatalog, 'description' | 'name' | 'unitOfMeasure'> &
  Partial<Pick<IMaterialWithdrawalItem, 'quantityWithdrawn'>>;

interface TableFormItemsMaterialWithdrawalProps {
  materialsInfo: IMaterialReceiptItemAddFormInfoWithdrawal[];
...
export function TableFormItemsMaterialWithdrawal({
  ...
  const infoMap = useMemo(() => {
    const map = new Map<number, IMaterialReceiptItemAddFormInfoWithdrawal>();
  ...
  const handleQuantityChange =
    ...
    (key: number, change: number) => {
      ...
      if (material) {
        const newQuantity = Number(material[type] || 0) + change;
        onUpdateItemQuantity(
          key,
  ...
  if (materials.length === 0) {
    return (
      <div className='text-muted-foreground py-8 text-center'>
        {readOnly
          ? 'Nenhum material para esta retirada.'
          : 'Nenhum material adicionado, utilize a consulta da listagem de retiradas para adicionar.'}
      </div>
    );
  }
                  ...
                  <td className='px-4 py-3 text-right text-sm'>
                    <div className='flex items-center justify-center gap-2'>
                      <Badge variant='outline'>
                        {Number(info?.quantityWithdrawn || 0).toLocaleString(
                          'pt-BR',
                          {
                  ...
                  <td className='px-4 py-3'>
                    {readOnly ? (
                      <p className='text-gray-900'>
                        {Number(material.quantityRejected || 0)}
                      </p>
                    ) : (
                      <div className='flex items-center justify-center gap-2'>
                        <Button
                          ...
                          disabled={Number(material.quantityRejected || 0) <= 0}
                        >
                        ...
                        <Input
                          type='number'
                          step='any'
                          value={String(material.quantityRejected || 0)}
                          onChange={(e) =>
                    ...
                    {readOnly ? (
                      ...
                      <div className='flex items-center justify-center gap-2'>
                        <Button
                          ...
                          disabled={Number(material.quantityReceived || 0) <= 0}
                        >
                          <Minus className='h-3 w-3' />
                        </Button>
                        <Input
                          type='number'
                          step='any'
                          value={String(material.quantityReceived || 0)}
                          onChange={(e) =>
