...
export async function getTimelineEvents(accessTokenSisman: string) {
  ...
  try {
    ...
    logger.info(`(Server Action) getTimelineEvents: ${data.length} timeline-events returned`);
    return data;
  } catch (error) {
    logger.error(`(Server Action) getTimelineEvents: Error fetching timeline-events`, error);
    throw error;
  }
}

export async function showTimelineEvent(accessTokenSisman: string, id: number) {
  logger.info(`(Server Action) showTimelineEvent: Fetching timeline-event ${id}`);
  try {
    ...
    logger.info(`(Server Action) showTimelineEvent: timeline-event ${id} returned`);
    return data;
  } catch (error) {
    logger.error(`(Server Action) showTimelineEvent: Error fetching timeline-event ${id}`, error);
    throw error;
  }
}

export async function getRefreshedTimelineEvents() {
  logger.info(`(Server Action) getRefreshedTimelineEvents: Revalidating ${PAGE_PATH}`);
  try {
    revalidatePath(PAGE_PATH);
    logger.info(`(Server Action) getRefreshedTimelineEvents: Path ${PAGE_PATH} revalidated`);
    return true;
  } catch (error) {
    logger.error(`(Server Action) getRefreshedTimelineEvents: Error revalidating path`, error);
  }
}
...
export async function addTimelineEvent(
  ...
  logger.info(`(Server Action) addTimelineEvent: Attempt to add timeline-event`, data);
...
export async function updateTimelineEvent(
  ...
  logger.info(`(Server Action) updateTimelineEvent: Attempt to update timeline-event ${data.id}`, data);
  ...
  try {
    ...
    logger.error(`(Server Action) updateTimelineEvent: Error updating timeline-event ${data.id}`, error);
    return {
      isSubmitSuccessful: false,
      errorsServer: ['An unexpected error occurred'],
      submittedData: data,
