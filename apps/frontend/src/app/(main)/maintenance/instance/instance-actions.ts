...
export async function getInstances(accessTokenSisman: string) {
  logger.info(`(Server Action) getInstances: Fetching instances`);
  try {
    const data = await fetchApiSisman(API_RELATIVE_PATH, accessTokenSisman, {
      cache: 'force-cache'
    });
    logger.info(`(Server Action) getInstances: ${data.length} instances returned`);
    return data;
  } catch (error) {
    logger.error(`(Server Action) getInstances: Error fetching instances`, error);
    throw error;
  }
}
...
export async function showInstance(accessTokenSisman: string, id: number) {
  ...
  try {
    ...
    logger.error(`(Server Action) showInstance: Error fetching instance ${id}`, error);
    throw error;
  }
}

export async function getRefreshedInstances() {
  logger.info(`(Server Action) getRefreshedInstances: Revalidating ${PAGE_PATH}`);
  try {
    revalidatePath(PAGE_PATH);
    logger.info(`(Server Action) getRefreshedInstances: Path ${PAGE_PATH} revalidated`);
    return true;
  } catch (error) {
    logger.error(`(Server Action) getRefreshedInstances: Error revalidating path`, error);
  }
}
...
export async function updateInstance(
  ...
  logger.info(`(Server Action) updateInstance: Attempt to update instance ${data.id}`, data);
  ...
  try {
    ...
    logger.error(`(Server Action) updateInstance: Error updating instance ${data.id}`, error);
    return {
      isSubmitSuccessful: false,
      errorsServer: ['An unexpected error occurred'],
      submittedData: data,
